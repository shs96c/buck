      ...
      try (Stream<Path> stream = Files.walk(source)) {
        stream.forEach(
            path -> {
              ...
              try {
                // Hardlinks are only for files - so, create directories. We can't use `MostFiles`
                // since that would introduce a circular dependency. Instead, we'll do this the old
                // fashioned way.
